...
1) 需求层（完整）
   - 给愿景文档加定位声明头："产品愿景叙事，不直接驱动开发；最终交付物＝产品"
...
4) 详细设计（完整骨架 + 当前阶段细节）
   ...
   - 08-dev-plan：当前阶段按功能范围与交付物形态拆 Sprint
   - 09-verification：测试策略 + REQ→用例追溯矩阵（当前阶段，含交付物形态）
...
13. 每份文档生成前用 1-2 句说结构；全部生成后用第 10 节 checklist 自查，单列"需人工确认项"
...
3. 不增加00-02中没有提及的功能
4. 明确标注当前 Phase 的功能范围、交付物形态（Demo/MVP/产品）以及允许和禁止的技术/功能边界
...
7. REQ 全覆盖：03 §1 列出 02-srs 全部 REQ；每个 REQ 在 03 §3 有阶段归属，并能追溯到 08 Sprint 与 09 验收用例；不得留下悬空 REQ
8. `docs/09-verification.md` 必须包含本机资源验证项，并在 REQ → 用例追溯矩阵或分阶段验证范围中显式写出交付物形态
9. 技术栈/实现状态必须区分「已用 / 预留·未启用 / 默认关闭」；不得把候选、预留或默认关闭写成已用
10. 对 ai/project-rules.md §3 已标记为“省略”的文档，不得生成空壳；应直接跳过，并在结果中说明跳过原因
11. `docs/05-tech-spec.md` 的“编码约定”一节若引用不到 ai/project-rules.md §5，可先写“待 03-09 审核后回填”，不要虚构内容
12. 每份文档生成前先用1-2句话说明本文档的结构安排
13. 生成后用第10节的验收 checklist 自查 03-09，列出未通过项再修订
```
...
- 03 §1 全部 REQ → 03 §3 阶段 → 08 Sprint → 09 验收用例可追溯，无悬空 REQ
