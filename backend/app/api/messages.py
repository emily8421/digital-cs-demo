...
def simulate(req: SimulateRequest, db: Session = Depends(get_db)):
    """模拟器投递一条入站消息 → 归一化 → 入库；返回 message_id / conversation_id。"""
    normalized = simulator_channel.receive(req.model_dump())
    message_id, conversation_id = handle_inbound(db, normalized, channel_name="simulator")
    return ApiResponse(data=SimulateData(message_id=message_id, conversation_id=conversation_id))
