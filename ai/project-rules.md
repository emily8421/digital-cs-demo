# 项目专属规则

> 本文件每个新项目都需要重新填写，不参与跨项目同步。
> 判断标准：一条规则换到另一个完全不同的项目上是否还成立——
> 不成立（涉及具体技术栈/具体功能/具体Phase定义）就属于本文件。
>
> 填写时机：§1 Phase边界、§2 技术栈、§3 项目形态与文档裁剪在生成 docs/03-09 **之前**填
> （作为约束）；§4 目录特例、§5 编码约定与禁区在审核 03-09 **之后**补。
>
> ⚠️ **本文件 §0–§3 由 AI 于文档体系初始化时填写为「草稿」，需人工确认。**
> 阶段标签的唯一来源是 `docs/03-prd.md` §3；确认/调整路线图后，按 INIT-PROMPT §4 同步本文件 §1 指针（需求不变，只动标签）。

## 0. 项目标识

- 项目名称：DigitalCustomerService_Demo（数字客服 · 演示）
- 代号/缩写：`dcs`（用于数据库表前缀 `dcs_`、后端包名 `digital_cs`、配置命名空间等）
- 业务角色名（虚构，仅供示例与文档引用）：智能助理「小辰」、企业「汇辰灯饰」、客服 小雯 / 技术 阿杰 / 跟单 老黄 / 经营者 陈总 / 新员工 小桐

## 1. Phase边界

> **Phase2（优化扩展）已于 2026-06-23 收官**——让回复更准、协作边界更稳、知识可自增长、时效可观测。
> 下一阶段＝远期愿景（REQ-15/16/17，待技术验证，**未启动**）；与 `docs/03-prd.md` §3 路线图一致。

当前阶段：P2（**交付物形态：Demo**）已收官；愿景（交付物形态：产品·最终，当前未启动）待技术验证。

**交付物形态**（v1.7.0 §8.1 双维度）：P1 / P2 均为 **Demo**——模拟器通道 + LLM 未启用 + 本机 Docker，无真实通道 / 生产要素；到 **MVP** 需真实通道 + 生产要素，属愿景。与 `docs/03-prd.md` §3 一致。

允许（P1 全部 + P2 新增）：
- 后端 Python + FastAPI（REST + Webhook 回调）
- 关系型数据库（PostgreSQL）持久化
- LLM + 向量检索（RAG）知识问答（P1 单轮；P2 增多轮）
- 消息通道抽象层 + **内置模拟器**
- 转人工后「AI 暂停」编排（**P1 会话级 → P2 话题（thread）级精化**，REQ-10）
- 非文字消息**类型识别 + 如实告知 + 提醒人工**（不涉及内容理解）
- 定时任务（定时小结 / 日报 / P2 时效扫描）
- 内部管理 API
- Docker 本地部署
- **多轮对话状态机**（定制询盘逐项引导，REQ-9）
- **AI 身份披露**（被问时按既定口径承认，REQ-11）
- **知识内容确认回写 + 知识确认轻量页面（前端）**（拍板人确认 → pending→confirmed，REQ-13）
- **响应时效 SLA 监控**（消息→首次应答间隔扫描、超时提示，REQ-14）

禁止（仍不做，留愿景）：
- 售后质保边界推理 / 安抚话术生成 —— 愿景·待技术验证
- 语音 / 图片 / 视频**内容**理解（永久禁止）
- 订单 / 生产记录系统集成 —— 愿景·待技术验证
- **企业微信真实通道接入**（Sprint-0 不成立，待替代方案选定）—— 愿景·待技术验证（**本期不接**）
- 对个人微信账号的**商用**非官方自动化操作（永久禁止）；非商用验证例外，受 DEC-7 边界约束（见 `docs/decisions/open-decisions.md` DEC-7）

下一阶段预告（愿景·待技术验证）：
- 企微替代通道（微信客服等）、订单进度查询、售后规则推理。

## 2. 技术栈约束

> 版本「待确认」处为 AI 草稿，落地前需人工/架构确认；不杜撰精确版本号。

- 后端语言/框架：Python + FastAPI（版本待确认）
- 数据库：PostgreSQL（版本待确认）
- 向量检索/RAG：**pgvector**（复用 PG，pg16；向量列 `vector(512)`）— 已定，免独立向量引擎
- LLM（对话生成）：**Demo 阶段未启用**——作答＝知识库检索 answer 原文直回（保「不编造」产品红线：无依据不生成事实，见 design-conversation-engine §2）；未来可在 RAG 限定内引入 LLM（润色/措辞，不生成新事实）；中转站 GLM-5.2/DeepSeek 为候选
- 消息平台：企业微信开放 API（⚠️ 客户群对外部用户收发能力 Sprint-0 已**证伪**，见 REQ-15 / `sprint-0-wework-findings.md`；真实通道待替代）
- 部署：Docker（docker/）
- 任务调度：**外部 cron**（应用不内嵌 APScheduler）— 已定（Sprint-6 收敛）

禁止引入的替代品：
- 个人微信的**商用**非官方自动化（pyautogui/协议库等）——永久禁止；非商用验证例外（见 DEC-7）
- 不在本期引入独立**功能**前端框架（通知走消息通道，见 §3）；演示辅助 UI（纯 HTML/JS 无框架，挂载 `/ui`）于 P1 收官后补，不在此列；如 P2 评估需要「知识确认」**功能**页面再议

### 2.5 运行环境与资源约束

> 本机环境事实见 `docs/env/local-env.md`（`collect-env.ps1` 自动采集 + 人工确认项）。以下待人工确认，不虚构：
- 本机 Demo 必须运行：后端（FastAPI/uvicorn）+ Docker（PG pgvector + TEI embedding）
- 允许降级/Mock：TEI 不可用时编排跳过检索（`try/except + logging.warning`）；SQLite 内存库用于单测（不依赖 Docker）
- 禁止本机重资源：进程内 torch（Python 3.14+Windows DLL 失败，已改 Docker TEI，见 `docs/env/context-and-constraints.md` §3）
- 是否允许联网：**Demo 不联网**（作答＝本地检索；拉 BGE 模型 / 中转站 LLM / 飞书 webhook 需联网属 MVP，Demo 默认本机）
- 是否允许安装依赖：**允许（已装齐）**（Docker Desktop + `.venv` 就绪；新增依赖须先确认，见 §5.2）
- 是否允许公司服务器 / 资源申请口径：**Demo 不需要**（本机原型先行；公司 Linux 服务器属 MVP / 部署，见 05 §1）

## 2.8 项目版本管理

> 模板仓的版本规则（「影响下游同步判断就 bump VERSION」，见同步来的 `MAINTAINERS.md` / `CONTRIBUTING.md`）面向模板维护者，对本项目不适用——本项目没有下游派生。本节定义 digital-cs-demo 自有的版本语义与递增规则。
> 版本入口：`VERSION`（项目自有版本）+ `CHANGELOG.md` 顶部「项目版本」段；模板继承版本独立记录在 `TEMPLATE-BASE.md`，不与项目版本混淆。

### 2.8.1 版本语义（三段式 vMAJOR.MINOR.PATCH）

- **PATCH**（v0.1.0 → v0.1.1）：bug 修复、文档修正、Demo 数据 / 配置调整、重构，不新增可演示能力、不改对外 API 契约。
- **MINOR**（v0.1.0 → v0.2.0）：Sprint 验收 / 里程碑交付、新增可演示能力、新增 API endpoint、Phase 内功能增强（如多轮状态机精化、知识确认回写）。默认向后兼容。
- **MAJOR**（v0.x → v1.0）：Phase 跨越（如 Demo → MVP，接入真实通道 / 生产要素）、破坏性对外契约变更、首个真实场景 / 试点上线。

### 2.8.2 何时 bump

- 完成一个 Sprint 验收 / 里程碑 → 至少 **MINOR**。
- 合并了用户 / 试点可感知的能力变化 → **MINOR**。
- 纯修复 / 文档 / 配置 / 重构 → **PATCH**。
- Phase 跨越、破坏性变更、首上线 → **MAJOR**。
- 不强制每个 commit bump；同一里程碑内的多个改动聚合为一个版本发布。
- 纯探索原型、研究记录、未确认提案不触发版本递增。

### 2.8.3 发布动作

1. 更新根目录 `VERSION` 为新版本号（三段式 `vX.Y.Z`）。
2. 在 `CHANGELOG.md` 顶部「项目版本」段（即 `## 历史模板同步记录（保留）` 之上）新增 `## vX.Y.Z（YYYY-MM-DD）` 条目，概述本版交付并附 REQ / Sprint / TC 追溯。
3. 版本递增属于状态变更，按 `ai/global-rules.md` / `ai/rules-core.md` 的写入确认规则先说明范围、风险与验证，AI 给建议、用户确认后再执行。
4. （可选）打 git tag `vX.Y.Z`；当前阶段不强制，不引入独立 release workflow。

### 2.8.4 CI 校验

`.github/workflows/project-check.yml` 在每次 PR / push 到 main 时校验：
- `VERSION` 符合三段式 `^v[0-9]+\.[0-9]+\.[0-9]+$`；
- `CHANGELOG.md` 顶部第一个 `## vX.Y.Z（` 标题等于 `VERSION`（确保项目版本在顶部且与 VERSION 一致）。
- 不做全文档降序检查——派生 CHANGELOG 顶部为项目版本（如 `v0.1.0`），下方接模板历史版本（`v1.30.4` 等，数值更大），降序不适用。

## 3. 项目形态与文档裁剪

- 是否有持久化存储：**有**（关系型数据库 PostgreSQL）
- 是否有对外接口：**有**（企业微信 Webhook 回调 + 内部管理 REST API）
- 演示形态：消息通道内交互（P1）+ 演示辅助 UI（`demo-ui` `/ui`）+ **P2 知识确认功能页面（前端，REQ-13）**
- `docs/06-db-design.md`：**保留**
- `docs/07-api-spec.md`：**保留**
- 需要保留的代码目录：`backend/`、`docker/`、`scripts/`、`tests/`
  - `frontend/`：**演示辅助 UI（`frontend/index.html` `/ui`，P1 后补）+ P2 知识确认功能页面（REQ-13）**。P1 功能交互走消息通道；P2 增知识确认轻量页面（拍板人确认回写）。

按项目形态裁剪说明：
- 06、07 均保留并已生成（有持久化、有对外接口）。
- `frontend/`：演示辅助 UI（`/ui`）+ P2 知识确认功能页面（REQ-13）；P1 功能走消息通道，P2 增功能前端。

## 4. 目录规范的项目特例

无重大差异，遵循 global-rules.md 通用目录标准。补充：
- 子系统详细设计进 `docs/design/<子系统>.md`（不占用 00-09 编号，见现行 `docs/README.md` 分区规则 + `docs/04-architecture.md` §2）。**不得把新增项目文档直接堆 `docs/` 根**（00-09/README 除外）。

## 5. 编码约定与禁区

> Phase 级功能禁止见 §1，技术栈替代品禁止见 §2，本节只管代码层。
> P1+P2 已收官（2026-06-23），本节反映代码现状（非占位）。错误处理现状＝编排层 `try/except + logging.warning` 优雅降级（TEI 不可用跳过检索等，见 `orchestrator.py`；打磨批次3 print→logging）；统一日志格式/异常类型规范待愿景阶段或专门 logging 任务细化。

### 5.1 既有约定（新代码必须向其看齐）
- 命名：后端 snake_case（含 DB 字段、函数、模块）；类 PascalCase；常量 UPPER_SNAKE。表前缀统一 `dcs_`。
- 分层与目录：`backend/` 建议 `api/`（接口层，仅进此层）→ `service/`（业务编排）→ `model/`（数据访问/领域模型）；通道接入归 `channels/`。
- 既有模式：消息处理统一走「通道适配 → 归一化消息 → 对话编排 → 出站」管线，新功能沿用，勿另起并行链路。
- 错误处理 / 日志：统一异常类型与统一日志格式（含会话 ID、发送方、消息类型）；待回填具体规范。

### 5.2 禁区（未经人工确认不得触碰）
- 不得擅改的文件 / 模块：通道适配层契约、路由规则默认数据、知识库种子数据、数据库迁移脚本。
- 不得擅自引入的依赖：**任何新依赖须先确认**（尤其 LLM/向量库/IM SDK 相关）。
- 不得自行实现的功能：见 §1「禁止」清单（多轮状态机、售后推理、知识回写、非文字内容理解、企业微信对外发送等）。
- 愿景 / 01 中的功能点不等于已批准实现；阶段归属以 `docs/03-prd.md` §3 路线图为准，编码以 §1 当前阶段为准。
